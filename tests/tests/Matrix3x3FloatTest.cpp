...
class Matrix3x3FloatTest : public ::testing::Test
{
    protected:
...
TEST_F(Matrix3x3FloatTest, constructors)
{
    // Default constructor
    Matrix3x3<FLOAT> m1;
...
TEST_F(Matrix3x3FloatTest, clear)
{
...
TEST_F(Matrix3x3FloatTest, setIdentity)
{
    Matrix3x3<FLOAT> m(11, 12, 13,
                       21, 22, 23,
                       31, 32, 33);
...
TEST_F(Matrix3x3FloatTest, inverse)
{
    Matrix3x3<FLOAT> inv;
    ...
    EXPECT_NEAR(0.0, inv[ 0], 1e-6);
    EXPECT_NEAR(0.0, inv[ 1], 1e-6);
    EXPECT_NEAR(0.0, inv[ 2], 1e-6);
    EXPECT_NEAR(0.0, inv[ 3], 1e-7);
    EXPECT_NEAR(0.0, inv[ 4], 1e-7);
    EXPECT_NEAR(0.0, inv[ 5], 1e-6);
    EXPECT_NEAR(0.0, inv[ 6], 1e-7);
    EXPECT_NEAR(0.0, inv[ 7], 1e-7);
    EXPECT_NEAR(0.0, inv[ 8], 1e-6);
}



TEST_F(Matrix3x3FloatTest, determinant)
{
    ...
    EXPECT_NEAR(319.0, n.determinant(), 1e-10);
    ...
    EXPECT_NEAR(0.0, o.determinant(), 1e-10);
}



TEST_F(Matrix3x3FloatTest, transpose)
{
    // Identity determinant
    Matrix3x3<FLOAT> m(11, 12, 13,
                       21, 22, 23,
...
TEST_F(Matrix3x3FloatTest, constants)
{
    // Identity matrix
    for (int i = 0; i < 9; ++i)
    {
        if (i == 0 || i == 4 || i == 8)
        {
            EXPECT_FLOAT_EQ(1.0f, Matrix3x3<FLOAT>::IDENTITY[i]);
        }
        else
        {
            EXPECT_FLOAT_EQ(0.0f, Matrix3x3<FLOAT>::IDENTITY[i]);
        }
    }
    ...
    for (int i = 0; i < 9; ++i)
    {
        EXPECT_FLOAT_EQ(0.0f, Matrix3x3<FLOAT>::ZERO[i]);
    }
}



TEST_F(Matrix3x3FloatTest, operators)
{
    Matrix3x3<FLOAT> m(11, 12, 13,
                       21, 22, 23,
                       31, 32, 33);
