...
class Vector2DFloatTest : public ::testing::Test
{
    protected:
...
TEST_F(Vector2DFloatTest, constructors)
{
    // Default constructor
    Vector2D<FLOAT> v;
...
TEST_F(Vector2DFloatTest, length)
{
    // v = (0, 0)
    Vector2D<FLOAT> v;
    ...
    EXPECT_NEAR(sqrt(2.0f), u.length(), 1e-7);
    ...
    EXPECT_NEAR(8.362439835359056, (double)y.length(), 1e-6);
}



TEST_F(Vector2DFloatTest, slength)
{
    // v = (0, 0)
    Vector2D<FLOAT> v;
    ...
    // u = (1, 1)
    Vector2D<FLOAT> u(1.0f, 1.0f);
    EXPECT_NEAR(2.0f, u.slength(), 1e-10);
    ...
    // y = (7.98, -2.5)
    Vector2D<FLOAT> y(7.98f, -2.5f);
    EXPECT_NEAR(69.9304, y.slength(), 1e-5);
}



TEST_F(Vector2DFloatTest, normalize)
{
    ...
    w1.normalize();
    EXPECT_NEAR(1.0f, w1.length(), 1e-10);
...
TEST_F(Vector2DFloatTest, scale)
{
    ...
    // u2 = (1, 1)
    u2.scale(5.f, 5.f);
    EXPECT_NEAR(7.0710678118654752440084436210484903928483593768847403, u2.length(), 1e-8);
    ...
    // u5 = (1, 1)
    u5.scale(5, 3);
    EXPECT_NEAR(std::sqrt(34.0), u5.length(), 1e-6);
}



TEST_F(Vector2DFloatTest, orientedArea)
{
...
TEST_F(Vector2DFloatTest, project)
{
    Vector2D<FLOAT> r;
    ...
    EXPECT_NEAR(0.5f, r.x, 1e-10);
    EXPECT_NEAR(1.5f, r.y, 1e-10);
}



TEST_F(Vector2DFloatTest, dotProduct)
{
    Vector2D<FLOAT> u1(0, 0), u2(1, 1), u3(2.23876f, 4.4444f), u4(234.9898f, -1.22343f), u5(-2345.777f, -7878.887f);

    EXPECT_NEAR(u1.x * u2.x + u1.y * u2.y, u1 * u2, 1e-10);
        
    EXPECT_NEAR(u3.x * u4.x + u3.y * u4.y, u3 * u4, 1e-10);
        
    EXPECT_NEAR(u4.x * u5.x + u4.y * u5.y, u4 * u5, 1e-10);
        
    EXPECT_NEAR(u5.x * u1.x + u5.y * u1.y, u5 * u1, 1e-10);
        
    EXPECT_NEAR(u5.x * u3.x + u5.y * u3.y, u5 * u3, 1e-10);
}



TEST_F(Vector2DFloatTest, operators)
{
    Vector2D<FLOAT> r;
...
TEST_F(Vector2DFloatTest, constants)
{
