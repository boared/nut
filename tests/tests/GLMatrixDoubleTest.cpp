...
class GLMatrixDoubleTest : public ::testing::Test
{
    protected:
...
TEST_F(GLMatrixDoubleTest, constructors)
{
    // Default constructor
    GLMatrix<FLOAT> m1;
...
TEST_F(GLMatrixDoubleTest, clear)
{
    ...
    for (int i = 0; i < 16; ++i)
    {
        EXPECT_DOUBLE_EQ(GLMatrix<FLOAT>::ZERO[i], m[i]);
    }
}



TEST_F(GLMatrixDoubleTest, setIdentity)
{
    GLMatrix<FLOAT> m(11, 12, 13, 14,
                      21, 22, 23, 24,
                      31, 32, 33, 34,
    ...
    for (int i = 0; i < 16; ++i)
    {
        EXPECT_DOUBLE_EQ(GLMatrix<FLOAT>::IDENTITY[i], m[i]);
    }
}



TEST_F(GLMatrixDoubleTest, isAffine)
{
    GLMatrix<FLOAT> m(11, 12, 13, 14,
                      21, 22, 23, 24,
                      31, 32, 33, 34,
...
TEST_F(GLMatrixDoubleTest, inverse)
{
    GLMatrix<FLOAT> inv;
...
TEST_F(GLMatrixDoubleTest, determinant)
{
...
TEST_F(GLMatrixDoubleTest, transpose)
{
    // Identity determinant
    GLMatrix<FLOAT> m(11, 12, 13, 14,
                      21, 22, 23, 24,
...
TEST_F(GLMatrixDoubleTest, setRotation)
{
    ...
    EXPECT_NEAR(-0.8660253, v.z, 1e-6);

    // v2
    v = m * v2;
    EXPECT_NEAR(0.56500512, v.x, 1e-6);
    EXPECT_NEAR(0.53033, v.y, 1e-7);
    EXPECT_NEAR(-1.5490381, v.z, 1e-8);
    
    // v3
    v = m * v3;
    EXPECT_NEAR(0.60085713, v.x, 1e-6);
    ...
    EXPECT_NEAR(-0.8660253, u.z, 1e-6);
    EXPECT_NEAR(1.00000000, u.w, 1e-20);

    // u2
    u = m * u2;
    EXPECT_NEAR(0.56500512, u.x, 1e-6);
    EXPECT_NEAR(0.53033, u.y, 1e-7);
    EXPECT_NEAR(-1.5490381, u.z, 1e-8);
    EXPECT_NEAR(1.00000000, u.w, 1e-20);
    
    // u3
    u = m * u3;
    EXPECT_NEAR(0.60085713, u.x, 1e-6);
    EXPECT_NEAR(6.19307422, u.y, 1e-7);
    EXPECT_NEAR(2.365249, u.z, 1e-7);
    EXPECT_NEAR(1.000000, u.w, 1e-20);
}



TEST_F(GLMatrixDoubleTest, setScale)
{
    ...
    // v1
    v = m * v1;
    EXPECT_NEAR(5, v.x, 1e-20);
    EXPECT_NEAR(0, v.y, 1e-20);
    EXPECT_NEAR(0, v.z, 1e-20);

    // v2
    v = m * v2;
    EXPECT_NEAR(0, v.x, 1e-20);
    EXPECT_NEAR(3, v.y, 1e-20);
    EXPECT_NEAR(0, v.z, 1e-20);
    ...
    // u1
    u = m * u1;
    EXPECT_NEAR(5, u.x, 1e-20);
    EXPECT_NEAR(0, u.y, 1e-20);
    EXPECT_NEAR(0, u.z, 1e-20);
    EXPECT_NEAR(1, u.w, 1e-20);

    // u2
    u = m * u2;
    EXPECT_NEAR(0, u.x, 1e-20);
    EXPECT_NEAR(3, u.y, 1e-20);
    EXPECT_NEAR(0, u.z, 1e-20);
    EXPECT_NEAR(1, u.w, 1e-20);
...
TEST_F(GLMatrixDoubleTest, setTranslation)
{
    Vector3D<FLOAT> v, v1(1, 0, 0), v2(0, 1, 0), v3(0, 0, 1), v4(1, 1, 1);
    Vector4D<FLOAT> u, u1(1, 0, 0, 1), u2(0, 1, 0, 1), u3(0, 0, 1, 1), u4(1, 1, 1, 1);
    ...
    // v1
    v = m * v1;
    EXPECT_NEAR(v1.x + 5, v.x, 1e-20);
    EXPECT_NEAR(v1.y + 3, v.y, 1e-20);
    EXPECT_NEAR(v1.z + 2, v.z, 1e-20);

    // v2
    v = m * v2;
    EXPECT_NEAR(v2.x + 5, v.x, 1e-20);
    EXPECT_NEAR(v2.y + 3, v.y, 1e-20);
    EXPECT_NEAR(v2.z + 2, v.z, 1e-20);
    
    // v3
    v = m * v3;
    EXPECT_NEAR(v3.x + 5, v.x, 1e-20);
    EXPECT_NEAR(v3.y + 3, v.y, 1e-20);
    EXPECT_NEAR(v3.z + 2, v.z, 1e-20);
    
    // v4
    v = m * v4;
    EXPECT_NEAR(v4.x + 5, v.x, 1e-20);
    EXPECT_NEAR(v4.y + 3, v.y, 1e-20);
    EXPECT_NEAR(v4.z + 2, v.z, 1e-20);
    
    // u1
    u = m * u1;
    EXPECT_NEAR(u1.x + 5, u.x, 1e-20);
    EXPECT_NEAR(u1.y + 3, u.y, 1e-20);
    EXPECT_NEAR(u1.z + 2, u.z, 1e-20);
    EXPECT_NEAR(1, u.w, 1e-20);

    // u2
    u = m * u2;
    EXPECT_NEAR(u2.x + 5, u.x, 1e-20);
    EXPECT_NEAR(u2.y + 3, u.y, 1e-20);
    EXPECT_NEAR(u2.z + 2, u.z, 1e-20);
    EXPECT_NEAR(1, u.w, 1e-20);
    
    // u3
    u = m * u3;
    EXPECT_NEAR(u3.x + 5, u.x, 1e-20);
    EXPECT_NEAR(u3.y + 3, u.y, 1e-20);
    EXPECT_NEAR(u3.z + 2, u.z, 1e-20);
    EXPECT_NEAR(1, u.w, 1e-20);

    // u4
    u = m * u4;
    EXPECT_NEAR(u4.x + 5, u.x, 1e-20);
    EXPECT_NEAR(u4.y + 3, u.y, 1e-20);
    EXPECT_NEAR(u4.z + 2, u.z, 1e-20);
    EXPECT_NEAR(1, u.w, 1e-20);
}



TEST_F(GLMatrixDoubleTest, rotate)
{
    ...
    // v1
    v = m * v1;
    EXPECT_NEAR(-4.47852706, v.x, 1e-8);
    EXPECT_NEAR(3.619496107, v.y, 1e-6);
    EXPECT_NEAR(3.339407205, v.z, 1e-6);
    
    // u1
    u = m * u1;
    EXPECT_NEAR(-4.47852706, u.x, 1e-8);
    EXPECT_NEAR(3.619496107, u.y, 1e-6);
    EXPECT_NEAR(3.339407205, u.z, 1e-6);
    EXPECT_NEAR(1.000000, u.w, 1e-20);
}



TEST_F(GLMatrixDoubleTest, scale)
{
    Vector3D<FLOAT> v, v1(1, 0, 0), v2(0, 1, 0), v3(0, 0, 1), v4(1, 1, 1);
    Vector4D<FLOAT> u, u1(1, 0, 0, 1), u2(0, 1, 0, 1), u3(0, 0, 1, 1), u4(1, 1, 1, 1);
    ...
    // v1
    v = m * v1;
    EXPECT_NEAR(v1.x * 5 * -10, v.x, 1e-20);
    EXPECT_NEAR(v1.y * 3 * 1, v.y, 1e-20);
    EXPECT_NEAR(v1.z * 2 * 4, v.z, 1e-20);

    // v2
    v = m * v2;
    EXPECT_NEAR(v2.x * 5 * -10, v.x, 1e-20);
    EXPECT_NEAR(v2.y * 3 * 1, v.y, 1e-20);
    EXPECT_NEAR(v2.z * 2 * 4, v.z, 1e-20);
    
    // v3
    v = m * v3;
    EXPECT_NEAR(v3.x * 5 * -10, v.x, 1e-20);
    EXPECT_NEAR(v3.y * 3 * 1, v.y, 1e-20);
    EXPECT_NEAR(v3.z * 2 * 4, v.z, 1e-20);
    
    // v4
    v = m * v4;
    EXPECT_NEAR(v4.x * 5 * -10, v.x, 1e-20);
    EXPECT_NEAR(v4.y * 3 * 1, v.y, 1e-20);
    EXPECT_NEAR(v4.z * 2 * 4, v.z, 1e-20);
    
    // u1
    u = m * u1;
    EXPECT_NEAR(u1.x * 5 * -10, u.x, 1e-20);
    EXPECT_NEAR(u1.y * 3 * 1, u.y, 1e-20);
    EXPECT_NEAR(u1.z * 2 * 4, u.z, 1e-20);
    EXPECT_NEAR(1, u.w, 1e-20);

    // u2
    u = m * u2;
    EXPECT_NEAR(u2.x * 5 * -10, u.x, 1e-20);
    EXPECT_NEAR(u2.y * 3 * 1, u.y, 1e-20);
    EXPECT_NEAR(u2.z * 2 * 4, u.z, 1e-20);
    EXPECT_NEAR(1, u.w, 1e-20);
    
    // u3
    u = m * u3;
    EXPECT_NEAR(u3.x * 5 * -10, u.x, 1e-20);
    EXPECT_NEAR(u3.y * 3 * 1, u.y, 1e-20);
    EXPECT_NEAR(u3.z * 2 * 4, u.z, 1e-20);
    EXPECT_NEAR(1, u.w, 1e-20);

    // u4
    u = m * u4;
    EXPECT_NEAR(u4.x * 5 * -10, u.x, 1e-20);
    EXPECT_NEAR(u4.y * 3 * 1, u.y, 1e-20);
    EXPECT_NEAR(u4.z * 2 * 4, u.z, 1e-20);
    EXPECT_NEAR(1, u.w, 1e-20);
}



TEST_F(GLMatrixDoubleTest, translate)
{
    Vector3D<FLOAT> v, v1(1, 0, 0), v2(0, 1, 0), v3(0, 0, 1), v4(1, 1, 1);
    Vector4D<FLOAT> u, u1(1, 0, 0, 1), u2(0, 1, 0, 1), u3(0, 0, 1, 1), u4(1, 1, 1, 1);
    ...
    v = m * v1;
    EXPECT_NEAR(v1.x + 5 - 10, v.x, 1e-20);
    EXPECT_NEAR(v1.y + 3 - 4, v.y, 1e-20);
    EXPECT_NEAR(v1.z + 2 - 6, v.z, 1e-20);

    // v2
    v = m * v2;
    EXPECT_NEAR(v2.x + 5 - 10, v.x, 1e-20);
    EXPECT_NEAR(v2.y + 3 - 4, v.y, 1e-20);
    EXPECT_NEAR(v2.z + 2 - 6, v.z, 1e-20);
    
    // v3
    v = m * v3;
    EXPECT_NEAR(v3.x + 5 - 10, v.x, 1e-20);
    EXPECT_NEAR(v3.y + 3 - 4, v.y, 1e-20);
    EXPECT_NEAR(v3.z + 2 - 6, v.z, 1e-20);
    
    // v4
    v = m * v4;
    EXPECT_NEAR(v4.x + 5 - 10, v.x, 1e-20);
    EXPECT_NEAR(v4.y + 3 - 4, v.y, 1e-20);
    EXPECT_NEAR(v4.z + 2 - 6, v.z, 1e-20);
    
    // u1
    u = m * u1;
    EXPECT_NEAR(u1.x + 5 - 10, u.x, 1e-20);
    EXPECT_NEAR(u1.y + 3 - 4, u.y, 1e-20);
    EXPECT_NEAR(u1.z + 2 - 6, u.z, 1e-20);
    EXPECT_NEAR(1, u.w, 1e-20);

    // u2
    u = m * u2;
    EXPECT_NEAR(u2.x + 5 - 10, u.x, 1e-20);
    EXPECT_NEAR(u2.y + 3 - 4, u.y, 1e-20);
    EXPECT_NEAR(u2.z + 2 - 6, u.z, 1e-20);
    EXPECT_NEAR(1, u.w, 1e-20);
    
    // u3
    u = m * u3;
    EXPECT_NEAR(u3.x + 5 - 10, u.x, 1e-20);
    EXPECT_NEAR(u3.y + 3 - 4, u.y, 1e-20);
    EXPECT_NEAR(u3.z + 2 - 6, u.z, 1e-20);
    EXPECT_NEAR(1, u.w, 1e-20);

    // u4
    u = m * u4;
    EXPECT_NEAR(u4.x + 5 - 10, u.x, 1e-20);
    EXPECT_NEAR(u4.y + 3 - 4, u.y, 1e-20);
    EXPECT_NEAR(u4.z + 2 - 6, u.z, 1e-20);
    EXPECT_NEAR(1, u.w, 1e-20);
}



TEST_F(GLMatrixDoubleTest, constants)
{
    // Identity matrix
    for (int i = 0; i < 16; ++i)
    {
        if (i == 0 || i == 5 || i == 10 || i == 15)
        {
            EXPECT_DOUBLE_EQ(1.0, GLMatrix<FLOAT>::IDENTITY[i]);
        }
        else
        {
            EXPECT_DOUBLE_EQ(0.0, GLMatrix<FLOAT>::IDENTITY[i]);
        }
    }
    ...
    for (int i = 0; i < 16; ++i)
    {
        EXPECT_DOUBLE_EQ(0.0, GLMatrix<FLOAT>::ZERO[i]);
    }
    ...
    for (int i = 0; i < 16; ++i)
    {
        if (i == 15)
        {
            EXPECT_DOUBLE_EQ(1.0, GLMatrix<FLOAT>::ZEROAFFINE[i]);
        }
        else
        {
            EXPECT_DOUBLE_EQ(0.0, GLMatrix<FLOAT>::ZEROAFFINE[i]);
        }
    }
}
...
TEST_F(GLMatrixDoubleTest, operators)
{
    GLMatrix<FLOAT> m(11, 12, 13, 14,
                      21, 22, 23, 24,
                      31, 32, 33, 34,
    ...
    EXPECT_DOUBLE_EQ(11 + 12 + 13 + 14, v.x);
    EXPECT_DOUBLE_EQ(21 + 22 + 23 + 24, v.y);
    EXPECT_DOUBLE_EQ(31 + 32 + 33 + 34, v.z);
...
TEST_F(GLMatrixDoubleTest, setLookAt)
{
    GLMatrix<FLOAT> m;
    ...
    // Row 1
    EXPECT_NEAR(l[ 0], m[ 0], 1e-20);
    EXPECT_NEAR(l[ 4], m[ 4], 1e-20);
    EXPECT_NEAR(l[ 8], m[ 8], 1e-20);
    EXPECT_NEAR(l[12], m[12], 1e-20);

    // Row 2
    EXPECT_NEAR(l[ 1], m[ 1], 1e-20);
    EXPECT_NEAR(l[ 5], m[ 5], 1e-16);
    EXPECT_NEAR(l[ 9], m[ 9], 1e-15);
    EXPECT_NEAR(l[13], m[13], 1e-20);
    
    // Row 3
    EXPECT_NEAR(l[ 2], m[ 2], 1e-15);
    EXPECT_NEAR(l[ 6], m[ 6], 1e-16);
    EXPECT_NEAR(l[10], m[10], 1e-20);
    EXPECT_NEAR(l[14], m[14], 1e-20);
    
    // Row 4
    EXPECT_NEAR(l[ 3], m[ 3], 1e-20);
    EXPECT_NEAR(l[ 7], m[ 7], 1e-20);
    EXPECT_NEAR(l[11], m[11], 1e-20);
    EXPECT_NEAR(l[15], m[15], 1e-20);
}



TEST_F(GLMatrixDoubleTest, setFrustum)
{
    GLMatrix<FLOAT> m;
    ...
    // Row 1
    EXPECT_NEAR(zNear / (width / 2.0),        m[ 0], 1e-20);
    EXPECT_NEAR(0,                            m[ 4], 1e-20);
    EXPECT_NEAR((left + right) / width, m[ 8], 1e-20);
    EXPECT_NEAR(0,                            m[12], 1e-20);
    
    // Row 2
    EXPECT_NEAR(0,                               m[ 1], 1e-20);
    EXPECT_NEAR(zNear / (height / 2.0),          m[ 5], 1e-20);
    EXPECT_NEAR((top + bottom) / height, m[ 9], 1e-20);
    EXPECT_NEAR(0,                               m[13], 1e-20);
    
    // Row 3
    EXPECT_NEAR(0,                                    m[ 2], 1e-20);
    EXPECT_NEAR(0,                                    m[ 6], 1e-20);
    EXPECT_NEAR(-( (zFar + zNear) / (zFar - zNear) ), m[10], 1e-20);
    EXPECT_NEAR((-2.0*zFar*zNear) / (zFar - zNear),    m[14], 1e-20);
    
    // Row 4
    EXPECT_NEAR(0,  m[ 3], 1e-20);
    EXPECT_NEAR(0,  m[ 7], 1e-20);
    EXPECT_NEAR(-1, m[11], 1e-20);
    EXPECT_NEAR(0,  m[15], 1e-20);
}



TEST_F(GLMatrixDoubleTest, setOrtho)
{
    GLMatrix<FLOAT> m;
    ...
    // Row 1
    EXPECT_NEAR(1.0 / (width / 2.0),     m[ 0], 1e-20);
    EXPECT_NEAR(0,                       m[ 4], 1e-20);
    EXPECT_NEAR(0,                       m[ 8], 1e-20);
    EXPECT_NEAR(-(right + left) / width, m[12], 1e-20);
    
    // Row 2
    EXPECT_NEAR(0,                        m[ 1], 1e-20);
    EXPECT_NEAR(1.0 / (height / 2.0),     m[ 5], 1e-20);
    EXPECT_NEAR(0,                        m[ 9], 1e-20);
    EXPECT_NEAR(-(top + bottom) / height, m[13], 1e-20);

    // Row 3
    EXPECT_NEAR(0,                                m[ 2], 1e-20);
    EXPECT_NEAR(0,                                m[ 6], 1e-20);
    EXPECT_NEAR(-1.0 / ((zFar - zNear) / 2.0),    m[10], 1e-20);
    EXPECT_NEAR(-(zFar + zNear) / (zFar - zNear), m[14], 1e-20);
    
    // Row 4
    EXPECT_NEAR(0, m[ 3], 1e-20);
    EXPECT_NEAR(0, m[ 7], 1e-20);
    EXPECT_NEAR(0, m[11], 1e-20);
    EXPECT_NEAR(1, m[15], 1e-20);
}
