...
class Vector3DDoubleTest : public ::testing::Test
{
    protected:
...
TEST_F(Vector3DDoubleTest, constructors)
{
    // Default constructor
    Vector3D<FLOAT> v;
...
TEST_F(Vector3DDoubleTest, length)
{
    // v = (0, 0, 0)
    Vector3D<FLOAT> v;
    ...
    EXPECT_NEAR(sqrt(3.0f), u.length(), 1e-20);
    ...
    EXPECT_NEAR(8.933052235938173444707, (double)y.length(), 1e-7);
}



TEST_F(Vector3DDoubleTest, slength)
{
    // v = (0, 0, 0)
    Vector3D<FLOAT> v;
    ...
    // u = (1, 1, 1)
    Vector3D<FLOAT> u(1.0f, 1.0f, 1.0f);
    EXPECT_NEAR(3.0f, u.slength(), 1e-20);
    ...
    EXPECT_NEAR(7.98f*7.98f + (-2.5f)*(-2.5f) + (3.1415f)*(3.1415f), y.slength(), 1e-6);
}



TEST_F(Vector3DDoubleTest, normalize)
{
    ...
    // u2 = (1, 1, 1)
    u2.normalize();
    EXPECT_NEAR(1.0f, u2.length(), 1e-20);
    ...
    // u5 = (7.98, -2.5, 3.1415)
    u5.normalize();
    EXPECT_NEAR(1.0f, u5.length(), 1e-15);
    ...
    w3.normalize();
    EXPECT_NEAR(1.0f, w3.length(), 1e-20);
...
TEST_F(Vector3DDoubleTest, scale)
{
    ...
    // u2 = (1, 1, 1)
    u2.scale(5, 3, 2);
    EXPECT_NEAR(std::sqrt(38.0), u2.length(), 1e-10);
...
TEST_F(Vector3DDoubleTest, crossProduct)
{
    ...
    EXPECT_NEAR(0.0f, r.z, 1e-20);

    // u2 x u3
    r = u2.cross(u3);
    EXPECT_NEAR(0.0f, r.x, 1e-20);
    EXPECT_NEAR(0.0f, r.y, 1e-20);
    EXPECT_NEAR(1.0f, r.z, 1e-20);

    // u3 x u4
    r = u3.cross(u4);
    EXPECT_NEAR(-2.0f, r.x, 1e-20);
    EXPECT_NEAR( 0.0f, r.y, 1e-20);
    EXPECT_NEAR(-3.5f, r.z, 1e-20);
    ...
    // u4 x u5
    r = u4.cross(u5);
    EXPECT_NEAR(-15772.455574512482, r.x, 1e-10);
    EXPECT_NEAR( 4649.55419f, r.y, 1e-10);
    EXPECT_NEAR(-30445.999389547535, r.z, 1e-10);
}



TEST_F(Vector3DDoubleTest, project)
{
    Vector3D<FLOAT> r;
    ...
    EXPECT_NEAR(17.0 / 26.0, r.x, 1e-15);
    EXPECT_NEAR(51.0 / 26.0, r.y, 1e-15);
    EXPECT_NEAR(34.0 / 13.0, r.z, 1e-15);
}



TEST_F(Vector3DDoubleTest, dotProduct)
{
    Vector3D<FLOAT> u1(0, 0, 0), u2(1, 1, 1), u3(2.23876f, 4.4444f, 3), u4(234.9898f, -1.22343f, -6), u5(-2345.777f, -7878.887f, 13.777);

    EXPECT_NEAR(u1.x * u2.x + u1.y * u2.y + u1.z * u2.z, u1 * u2, 1e-20);
        
    EXPECT_NEAR(u3.x * u4.x + u3.y * u4.y + u3.z * u4.z, u3 * u4, 1e-20);
        
    EXPECT_NEAR(u4.x * u5.x + u4.y * u5.y + u4.z * u5.z, u4 * u5, 1e-20);
        
    EXPECT_NEAR(u5.x * u1.x + u5.y * u1.y + u5.z * u1.z, u5 * u1, 1e-20);
        
    EXPECT_NEAR(u5.x * u3.x + u5.y * u3.y + u5.z * u3.z, u5 * u3, 1e-20);
}



TEST_F(Vector3DDoubleTest, operators)
{
    Vector3D<FLOAT> r;
...
TEST_F(Vector3DDoubleTest, constants)
{
